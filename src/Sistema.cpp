...
namespace ufmg_carona {
    ...
    Sistema::Sistema() : _usuario_logado(nullptr) {
        carregar_dados_iniciais(); // Carrega dados de arquivos
    }

    // Destrutor do sistema
    Sistema::~Sistema() {
        std::cout << "\nFinalizando o sistema..." << std::endl;
        salvar_dados_usuarios(); // Salva dados antes de fechar
        // Libera a memória alocada para usuários e solicitações
    ...
    void Sistema::executar() {
        ...
        std::string comando;
        while (true) {
            exibir_menu(); // Mostra as opções de comando
            std::cout << "> ";
            std::getline(std::cin, comando);
            if (comando == "sair" || std::cin.eof()) break; // Sai do programa
            if (comando.empty()) continue;
            try {
                processar_comando(comando); // Processa o comando digitado
    ...
    // Processa os comandos inseridos pelo usuário
    void Sistema::processar_comando(const std::string& comando) {
        if (!_usuario_logado) { // Se ninguém estiver logado
            if (comando == "cadastro") fluxo_cadastro(); // Entra no fluxo de cadastro
            else if (comando == "login") fluxo_login(); // Entra no fluxo de login
            else throw ComandoInvalidoException(comando);
        } else { // Se um usuário estiver logado
    ...
    // Gerencia o fluxo de cadastro de um novo usuário
    void Sistema::fluxo_cadastro() {
        ...
        // Busca dados do CPF no arquivo da UFMG
        ...
        // Valida se os dados da UFMG estão completos
        ...
        // Se desejar ser motorista, cadastra o veículo
        ...
        salvar_dados_usuarios(); // Salva os usuários atualizados
    ...
    void Sistema::fluxo_login() {
        ...
        Usuario* u = buscar_usuario_por_cpf(cpf); // Busca o usuário pelo CPF
        if (u && u->verificar_senha(senha)) { // Verifica se encontrou e a senha está correta
            _usuario_logado = u; // Define o usuário logado
    ...
    void Sistema::fluxo_logout() {
        _usuario_logado = nullptr; // Desloga o usuário
        std::cout << "Logout efetuado." << std::endl;
    }

    // Gerencia o fluxo de cadastro de veículo
    ...
    // Gerencia o fluxo de oferecimento de caronas
    void Sistema::fluxo_oferecer_carona() {
        ...
        _caronas.push_back(nova_carona); // Adiciona a carona à lista
        std::cout << "Carona (ID: " << nova_carona.get_id() << ") criada com sucesso!" << std::endl;
    }

    // Exibe as caronas disponíveis
    ...
    void Sistema::exibir_menu() {
        if (_usuario_logado) { // Menu para usuário logado
        ...
        } else { // Menu para usuário não logado
    ...
    // Busca dados de um usuário no arquivo dados_ufmg.txt
    ...
    // Carrega dados iniciais dos arquivos de texto
    void Sistema::carregar_dados_iniciais() {
        ...
        if (arquivo_usuarios.is_open()) {
            ...
            while (std::getline(arquivo_usuarios, linha)) {
                ...
                // Leitura dos campos do arquivo de usuários
                std::getline(ss, cpf, ';');
                std::getline(ss, nome, ';');
                ...
                if (novo_usuario_carregado) {
                    ...
                    // Atribui um veículo padrão se for motorista e não tiver veículo
        ...
        if (arquivo_caronas.is_open()) {
            ...
            while (std::getline(arquivo_caronas, linha)) {
                ...
                if (motorista_ptr) {
                    // Atribui veículo padrão ao motorista se ele não tiver um
    ...
    // Salva os dados dos usuários no arquivo de texto
    ...
    // Gerencia o fluxo de solicitação de caronas
    ...
    // Gerencia o fluxo de solicitações para motoristas
    void Sistema::fluxo_gerenciar_solicitacoes() {
        ...
        if (resposta == 's' || resposta == 'S') {
            solicitacao_escolhida->aceitar(); // Aceita a solicitação
            ...
            solicitacao_escolhida->recusar(); // Recusa a solicitação
            enviar_notificacao(solicitacao_escolhida->get_passageiro(),
    ...
    // Exibe o status das caronas para o usuário logado
    ...
    // Verifica se o passageiro pode solicitar a carona
